...
/*   Updated: 2024/03/11 17:58:23 by fwahl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*construct_expanded_content(const char *content, t_env *env)
{
	char	*var_name;
	char	*var_value;
	char	*new_content;
...
void	resolve_tokens(t_token **token_head, t_env *env)
{
	...
	while (current != NULL)
	{
		...
		{
			expanded_content = construct_expanded_content(current->content, env);
