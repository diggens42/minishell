...
/*   Updated: 2024/03/20 00:14:24 by fwahl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*loop_dir(char *cwd, struct dirent *ent, char **parts, char *res)
{
	char	*filename;
	char	*path;
	bool	match;
...
static char	*match_dir_entries(DIR *dir, char **parts, char *cwd)
{
	char			*res;
	struct dirent	*ent;
	...
	while (true)
	{
		...
		if (ent == NULL)
			break ;
		res = loop_dir(cwd, ent, parts, res);
	}
	return (res);
}
...
char	*expand_wildcard(char *content)
{
	...
	res = match_dir_entries(dir, parts, cwd);
	closedir(dir);
	free(cwd);
