...
/*   Updated: 2024/03/27 16:40:53 by fwahl            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_ast	*new_ast_node(t_token *token)
{
	...
	if (is_cmd(token->type) || is_redirect(token->type))
	{
		...
		node->cmd->argv = (char **)ft_calloc(n_cmd(token), sizeof(char *));
		if (is_redirect(token->type))
			node->cmd->redir = (t_redir **)ft_calloc(n_red(token), sizeof(t_redir));
	}
	return (node);
}
